...
const Navbar = () => {
  ...
  return (
    <div className='w-[100%] flex flex-row justify-between mx-auto py-8 px-6 fixed top-0 mr-5'>
        ...
        ): <button onClick={()=>navigate('/login')} className='flex items-center gap-2 bg-slate-300 shadow-sm shadow-slate-500 rounded-full py-2 px-4 text-lg'>Login<FaArrowRight/></button>}
    </div>
  )
}
